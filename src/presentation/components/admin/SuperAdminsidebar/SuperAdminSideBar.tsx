...
import {  Box, Flex, Text, Modal, Button, Image, Avatar } from '@mantine/core';
    ...
    return item.link ? (
        ...
        <div key={index} className={'linkside logOutLink'} onClick={openLogoutModal}>
          {IconComponent && (
            <span className={'linkIcon'}>
              <IconComponent stroke={1.5} variant="Bulk" color={iconColor} size={isCollapsed && isMobile ? 25 : 18} /> {/* Adjust size when collapsed and on mobile */}
            </span>
          )}
      ...
      item.link=='apploader'?  (
        <NavLink
          ...
          className={({ isActive }) =>
            `linkside ${isCollapsed ? 'centered' : ''} ${isActive && !disabledLabels.includes(item?.label || '') ? 'active' : ''}`
          }
          key={index}
          onClick={() => {
            if (isMobile) {
        ...
        >
          { (
            <span className={'linkIcon'}>
              <Avatar src={'https://gravatar.com/avatar/a21aebba7c9109b1c0945a2075669c8d?s=400&d=robohash&r=x'} size={isCollapsed ? 25 : 23} /> {/* Adjust size when collapsed and on mobile */}
            </span>
          )}
      ...
      (
        ...
        >
          {IconComponent && (
            <span className={'linkIcon'}>
              <IconComponent stroke={1.5} variant="Bulk" color={iconColor} size={isCollapsed ? 25 : 23} /> {/* Adjust size when collapsed and on mobile */}
            </span>
          )}
          {!isCollapsed && <p className={'textlink'}>{item.label}</p>}
        </NavLink>
  ...
  return (
    <Box className={`Sidenavbar ${isCollapsed ? 'collapsed' : ''}`}>
      <Box className={'SidenavbarMain'}>
        <Flex p={'1em'} justify={'center'} onClick={() => {
          if (isMobile) {
            onClose();
          }
        }}>
          ...
          />
            ...
            {
              isMobile&&( <Box className='buttonham'>
      ...
      {logoutModalOpened && (
        ...
        >
          ...
          <Flex mt="md" justify="flex-end">
            <Button onClick={closeLogoutModal} variant="default" mr="sm">
              Cancel
            </Button>
            <Button onClick={confirmLogout} color="black">
              Log Out
            </Button>
          </Flex>
        </Modal>
